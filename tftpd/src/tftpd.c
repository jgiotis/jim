  ...
  while ((opt = getopt (argc, argv, "dh:p:P:a:s:")) != -1)	/* this function is handy */
  {
      switch (opt)
      {
      	  case 'p':		/* path (opt required) */
      		  ...
      		  {
      			  printf ("Sorry, you specified an invalid/non-existant directory. Make sure the directory exists.\n");
      			  return 0;
      		  }
      	  ...
      	  case 'a':		/* ack frequency (opt required) */
      		  ...
      		  {
      			  printf ("Sorry, you specified an ack frequency higher than the maximum allowed (Requested: %d Max: %d)\n", ackfreq, MAXACKFREQ);
      			  return 0;
      		  }
      		  else if (ackfreq == 0)
      		  {
      	  ...
      	  case 's':		/* File chunk size (opt required) */
      		  ...
      		  {
      			  printf ("Sorry, you specified a data size higher than the maximum allowed (Requested: %d Max: %d)\n", datasize, MAXDATASIZE);
      			  return 0;
      		  }
      		  break;
      	  case 'h':		/* Help (no opts) */
  ...
  while (1)
  {
      ...
      while (errno == EAGAIN || n == 0)	/* This loop is required because on linux we have to acknowledge complete children with waitpid. Ugh. */
      {
    	  waitpid (-1, &status, WNOHANG);
	   	  n = recvfrom (sock, buf, BUFSIZ, MSG_DONTWAIT, (struct sockaddr *) &client, (socklen_t *) & client_len);
	   	  if (n < 0 && errno != EAGAIN)
	   	  {
...
tget (char *pFilename, struct sockaddr_in client, char *pMode, int tid)
{
  ...
  if (strchr (filename, 0x5C) || strchr (filename, 0x2F))	//look for illegal characters in the filename string these are \ and /
  {
      ...
      len = sprintf ((char *) packetbuf, "%c%c%c%cIllegal filename.(%s) You may not attempt to descend or ascend directories.%c",
  ...
  {				//if the pointer is null then the file can't be opened - Bad perms
      ...
      len = sprintf ((char *) packetbuf, "%c%c%c%cFile cannot be opened for writing (%s)%c", 0x00,
...
tsend (char *pFilename, struct sockaddr_in client, char *pMode, int tid)
{
...
#if 0 //JCG: allow traversing
  if (strchr (filename, 0x5C) || strchr (filename, 0x2F))	//look for illegal characters in the filename string these are \ and /
  {
    if (debug)
    	printf ("Server requested bad file: forbidden name\n");
    len = sprintf ((char *) packetbuf[0], "%c%c%c%cIllegal filename.(%s) You may not attempt to descend or ascend directories.%c", 0x00, 0x05, 0x00, 0x00, filename, 0x00);
    if (sendto (sock, packetbuf[0], len, 0, (struct sockaddr *) &client, sizeof (client)) != len)	/* send the data packet */
	{
	  printf ("Mismatch in number of sent bytes while trying to send error packet\n");
	}
  ...
  {				//if the pointer is null then the file can't be opened - Bad perms OR no such file
      ...
      len = sprintf ((char *) packetbuf[0], "%c%c%c%cFile not found (%s)%c", 0x00, 0x05, 0x00, 0x01, fullpath, 0x00);
      if (sendto (sock, packetbuf[0], len, 0, (struct sockaddr *) &client, sizeof (client)) != len)	/* send the data packet */
      {
    	  printf ("Mismatch in number of sent bytes while trying to send error packet\n");
      }
...
/* The following 'for' loop is used to recieve/timeout ACKs */
	for (j = 0; j < RETRIES; j++)
	{
	    ...
	    else
		{
		  ...
		  {
		      if (debug)
		    	  printf ("Error recieving ACK (ACK from invalid address)\n");
		  ...
		  if (tid != ntohs (client.sin_port))	/* checks to ensure get from the correct TID */
		  {
		      if (debug)
		    	  printf ("Error recieving file (data from invalid tid)\n");
		      len = sprintf ((char *) recvbuf, "%c%c%c%cBad/Unknown TID%c", 0x00, 0x05, 0x00, 0x05, 0x00);
		  ...
		  {
		      if (debug)
		    	  printf ("Remote host failed to ACK proper data packet # %d (got OP: %d Block: %d)\n", count, opcode, rcount);
		      /* sending error message */
		      if (opcode > 5)
			  {
